...
export default function SalesNew() {
  ...
  const totals = useMemo(() => {
    ...
    const iva = 0; // ajustar si corresponde
  ...
  const canSave = useMemo(() => {
    ...
    const valid = items.filter(
      (it) => (it.productLabel.trim() || it.productId) && number(it.qty) > 0
    );
  ...
  const addItem = () => {
    ...
    setItems([
      ...items,
      { key: maxKey + 1, productId: null, productLabel: '', qty: 1, price: 0 },
    ]);
  };
  ...
  const onSubmit = async () => {
    // ⚠️ Corrección: usar "date" (no "fecha") para alinear con lo que devuelve /orders
    const payload = {
      clientId,
      ...
      items: items
        .filter(
          (it) =>
            (it.productId || it.productLabel.trim()) && number(it.qty) > 0
        )
        .map((it) => ({
    ...
    try {
      // Backend final debería exponer POST /sales; si no existe, mostramos preview
      ...
      alert(
        'No se pudo enviar (¿/sales listo?). JSON:\n\n' +
              ...
              <tr key={it.key} style={{ borderTop: '1px solid #e2e8f0' }}>
                <td style={{ padding: '8px 8px' }}>
                  <ProductTypeahead
                    ...
                    onChange={(txt) =>
                      updateItem(it.key, { productLabel: txt })
                    }
                    onSelect={(id, label, price) => {
                      ...
                      if (!number(it.price) && number(price) > 0)
