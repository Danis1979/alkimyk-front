...
import { searchClients } from '../../services/masters.service';
...
export default function Clients() {
  ...
  const { data, isLoading, isError, error, isFetching } = useQuery({
    queryKey: ['masters.clients', { q, page, limit }],
    queryFn: () => searchClients({ q, page, limit }),
          ...
          <tbody>
            {items.map((c) => (
              <tr key={c.id} style={{ borderTop: '1px solid #e2e8f0' }}>
                ...
                <td style={{ padding: '10px 12px' }}>{c.taxId}</td>
