...
export default function Sales() {
  ...
  const limit = [10, 20, 50].includes(parsedLimit) ? parsedLimit : 20;
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey,
    queryFn: () => searchOrders({ page, limit, sort, q }),
    keepPreviousData: true,
  });

  const items = data?.items ?? [];
  const total = data?.total; // puede venir undefined si el backend no lo manda
  ...
  const fromRow = (page - 1) * limit + (items.length > 0 ? 1 : 0);
  const toRow = (page - 1) * limit + items.length;
  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto' }}>
      ...
      {/* Estado */}
      {isError && (
        <div style={{ color: '#b91c1c', marginBottom: 8 }}>Error cargando ventas.</div>
      )}
      {isLoading && (
      ...
      >
        <div style={{ fontSize: 12, color: '#475569' }}>
          {total != null
            ? `Mostrando ${fromRow || 0}–${toRow || 0} de ${total} · Página ${page}${pages ? ` de ${pages}` : ''}`
